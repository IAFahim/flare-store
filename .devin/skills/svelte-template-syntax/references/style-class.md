...
```svelte
<!-- 等价 --><div style:color="red">...</div><div style="color: red;">...</div>
```

### 表达式

```svelte
<div style:color={myColor}>...</div><div style:color>...</div> <!-- 简写 -->
```

### 多个

```svelte
<div style:color style:width="12rem" style:background-color={dark ? 'black' : 'white'}>...</div>
```
...
```svelte
<Button class={{ 'bg-blue-700 sm:w-1/2': useTailwind }} class:px-4={size === 'md'}>...</Button>
```

### 通过 props 透传 class

```svelte
<button {...props} class={['btn', 'btn-primary', props.class]}>...</button>
```
