...
<button onclick={() => (count += 1)}>+1</button><p>{count}</p>
```
...
```svelte
<div onclick={() => (count += 1)}>
	<button onclick={(e) => e.stopPropagation()}> Don't increment </button>
</div>
```
...
<input onkeydown={handleKey} placeholder="Press Enter to submit" />
```
