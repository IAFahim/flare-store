...
<div class={{ active: isActive, large: isLarge }}>Badge</div>
...
<div class={['text', isFaded && 'faded', isUnderline && 'underline', 'highlight']}>
	Array class example
</div>
...
```svelte
<div class:active class:large={isLarge}>Content</div>
```
...
<div class="tab" class:active={isActive} class:disabled={isDisabled}>Tab Content</div>
...
type ClassValue =
	string | number | boolean | undefined | null | ClassValue[] | { [key: string]: unknown };
```
...
<script lang="ts">
	...
	let { class: cls = '', ...props }: { class?: ClassValue } = $props();
</script>
...
<div class:active={isActive} class:large={isLarge}>Content</div>
```
...
<div class={{ active: isActive, large: isLarge }}>Content</div>
```
