...
The exact form of the suffix varies across Svelte versions, but the _contract_ is: within the same component, `wiggle` and `animation: wiggle` always refer to the same name; across components, the names never collide.
...
<style>
	...
	@keyframes -global-  spin {
		/* WRONG: extra space, won't be recognized as a global keyframe */
	}
	@keyframes shimmer {
		/* scoped, not global */
	}
...
```css
@keyframes spin-svelte-abc123 {
	to {
		transform: rotate(360deg);
	}
}
@keyframes pulse-svelte-abc123 {
	50% {
		opacity: 0.5;
	}
}
.spinner.svelte-abc123 {
	animation: spin-svelte-abc123 1s linear infinite;
}
.pulse.svelte-abc123 {
	animation: pulse-svelte-abc123 1.2s ease-in-out infinite;
}
```
