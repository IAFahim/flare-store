...
```svelte
<!-- Equivalent --><div style:color="red">A</div><div style="color: red">B</div>
```
...
Custom properties (CSS variables) are different from regular properties: every value is a string, and the browser only validates them when the variable is _consumed_ via `var()`.
