...
A _compound_ selector (`a.b`, `a.b.c`, `tag.class`, etc.) inside `:global(...)` is treated as a single unit. The **element** still needs to belong to the component's scope (the compiler adds the scope class to it), but the trigger class(es) can be added externally — even programmatically.
...
This is by design: `:global(.big)` opts `.big` out of _scoping_, not out of _specificity_.
...
```css
.card.svelte-xyz .body p.svelte-xyz {
	color: blue;
}
```
...
- It _does_ affect the `>` (child) combinator — a parent rule like `.parent > *` will match the wrapper, not the actual component.
