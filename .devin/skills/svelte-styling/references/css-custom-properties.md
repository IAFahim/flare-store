...
<DarkTheme>
	<Card />
	<!-- Card inherits dark theme variables -->
</DarkTheme>
```
...
<div style="--card-bg: #e8f5e9; --card-shadow: 0 4px 12px;">
	<Card />
	<!-- All Cards here inherit green theme -->
</div>
```
...
<div class="controls">
	<div
		class="color-preview"
		style={`--hue: ${hue}; --sat: ${saturation}%; --light: ${lightness}%;`}
	></div>
...
```svelte
<ProgressBar bind:value={progress} --progress-color="#22c55e" --progress-height="12px" />
```
...
```svelte
<div class="container">
	<div class="content">Responsive padding via CSS variable!</div>
</div>
...
<!-- Button.svelte -->
<script>
	let { variant = 'primary', size = 'medium', class: cls = '', ...props } = $props();
...
<button class={['btn', cls]} style={`${variants[variant]}; ${sizes[size]}`} {...props}>
	<slot />
</button>
