...
<div class="w-[calc(100%-2rem)] p-[1.5rem]">Arbitrary width and padding</div>

<!-- Arbitrary colors -->
<button class="bg-[#1da1f2] text-[#ffffff]"> Custom color button </button>

<!-- Arbitrary Tailwind values -->
<div class="text-[clamp(1rem,2vw,2rem)]">Responsive text with clamp</div>
```
...
<div class="min-h-screen bg-white transition-colors dark:bg-gray-900">
	<nav class="flex items-center justify-between bg-gray-100 p-4 dark:bg-gray-800">
		...
		<button onclick={toggleDark} class="rounded bg-blue-500 px-4 py-2 text-white hover:bg-blue-600">
			Toggle Dark Mode
		</button>
	</nav>

	<main class="p-8">
		<h1 class="mb-4 text-3xl font-bold text-gray-900 dark:text-white">Tailwind Dark Mode</h1>
		<p class="text-gray-600 dark:text-gray-300">This text adapts to dark mode automatically.</p>
	</main>
</div>
...
```svelte
<!-- Usage -->
<Button variant="primary" class="ml-2">Primary with extra margin</Button>

<Button variant="ghost" class="text-red-500 hover:text-red-600">Ghost with custom color</Button>
```
...
<!-- Card.svelte -->
<script>
	let { title, description, image, class: cls = '', ...props } = $props();
</script>

<div
	class={['card-shadow overflow-hidden rounded-xl bg-white', 'border border-gray-200', cls]}
	{...props}
>
	{#if image}
...
<div class="min-h-screen bg-gray-50">
	<!-- Mobile menu button -->
	<button class="p-4 md:hidden" onclick={() => (isMenuOpen = !isMenuOpen)}>
		<svg class="h-6 w-6">{isMenuOpen ? 'X' : 'Menu'}</svg>
	</button>

	<!-- Desktop navigation -->
	<nav class="hidden gap-4 bg-white p-4 shadow md:flex">
		{#each tabs as tab}
			<button
	...
	<!-- Mobile navigation -->
	{#if isMenuOpen}
		<nav class="bg-white p-4 shadow-lg md:hidden">
			{#each tabs as tab}
				<button
					onclick={() => {
						activeTab = tab;
	...
	<!-- Content -->
	<main class="p-8">
		<h1 class="text-2xl font-bold text-gray-900 md:text-4xl">
...
```svelte
<div class="h-[clamp(200px,40vh,400px)] w-[640px]">Fixed width, responsive height via clamp</div>

<div class="min-h-[100dvh]">Use dvh (dynamic viewport) for mobile-safe full screen</div>
```
...
<p class="tracking-[-0.02em]">Tighter-than-default tracking</p>
```
...
Bracket syntax also works in _variant_ position — `class-[is-active]:bg-blue-500` matches when the element has the class `is-active`. Useful for ad-hoc conditional styling without writing a custom variant.
...
<button onclick={toggle} class="rounded border px-3 py-1">
	{isDark ? 'Light' : 'Dark'}
</button>
...
The literal `bg-`, the dynamic `color`, and `-500` are _strings_ — the JIT cannot reconstruct the full class. Always prefer lookup maps or safelisting.
...
<button class={twMerge('bg-blue-500 p-2', cls)}>
	<slot />
</button>
```
