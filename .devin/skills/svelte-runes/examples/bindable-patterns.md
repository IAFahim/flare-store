...
<input type="number" {value} oninput={(e) => (value = clamp(+e.currentTarget.value))} />
```
...
<input type="color" bind:value={color} /><p>Selected: {color}</p>
```
...
<!-- TodoList.svelte -->
<script lang="ts">
	let { items = $bindable([]) }: { items: Array<{ id: number; text: string; done: boolean }> } =
		$props();
