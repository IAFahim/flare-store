...
<p>doubled: {doubled} (自动更新)</p><p>isBig: {isBig}</p>
```
...
<script>
	...
	let expensive = $derived.by(() => items.filter((i) => i.price > 5));
</script>
...
{:then items}
	<ul>
		{#each items as i}<li>{i.name}</li>{/each}
	</ul>
{:catch e}
	<p>error: {e.message}</p>
{/await}
...
<button onclick={() => (sum.value = 10)}>set a=8</button><p>sum: {sum.value}</p>
```
