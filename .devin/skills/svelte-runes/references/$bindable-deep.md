...
```svelte
let {((value = $bindable()), // 必填
(other = $bindable('hello')), // 带 fallback
(readonly = $bindable()))} = $props();
```
...
<input type="range" bind:value={() => value, (v) => (value = clamp(v))} {min} {max} />
```
...
<NumberField bind:value={count} min={0} max={10} /><p>current: {count}</p>
```
...
<!-- DateRange.svelte -->
<script lang="ts">
	let { start = $bindable(''), end = $bindable('') }: { start?: string; end?: string } = $props();
</script>
